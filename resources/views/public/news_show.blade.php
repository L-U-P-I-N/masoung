...
<div class="page-header">
    <div class="breadcrumb"><a href="{{ route('home') }}">الرئيسية</a> / <a href="{{ route('news') }}">الأخبار</a></div>
    <h1>{{ $item->title }}</h1>
</div>
            ...
            <div class="banner-slider" id="mainSlider">
                @foreach($images as $index => $image)
                    <div class="banner-item" id="slide-{{ $index }}">
                        <img src="{{ Storage::url($image) }}" alt="{{ $item->title }}" onclick="openLightbox({{ $index }})" style="cursor:zoom-in">
                    </div>
                @endforeach
            </div>
