...
<head>
    ...
    <title>@yield('title', 'قبيلة مسونق') | {{ $settings->tribe_name ?? 'قبيلة مسونق' }}</title>
    <meta name="description" content="@yield('description', 'الموقع الرسمي لقبيلة مسونق')">
...
<nav>
    <div class="nav-inner">
        <a href="{{ route('home') }}" class="nav-brand">⚜ {{ $settings->tribe_name ?? 'قبيلة مسونق' }} ⚜</a>
        <ul class="nav-links">
            <li><a href="{{ route('home') }}" class="{{ request()->routeIs('home') ? 'active' : '' }}">الرئيسية</a></li>
            <li><a href="{{ route('about') }}" class="{{ request()->routeIs('about') ? 'active' : '' }}">عن القبيلة</a></li>
            <li><a href="{{ route('members') }}" class="{{ request()->routeIs('members') ? 'active' : '' }}">الأعضاء</a></li>
            <li><a href="{{ route('activities') }}" class="{{ request()->routeIs('activities*') ? 'active' : '' }}">الأنشطة</a></li>
...
<div class="mobile-menu" id="mobileMenu">
    ...
    <a href="{{ route('home') }}" onclick="document.getElementById('mobileMenu').classList.remove('open')">الرئيسية</a>
    <a href="{{ route('about') }}" onclick="document.getElementById('mobileMenu').classList.remove('open')">عن القبيلة</a>
    <a href="{{ route('members') }}" onclick="document.getElementById('mobileMenu').classList.remove('open')">الأعضاء</a>
    <a href="{{ route('activities') }}" onclick="document.getElementById('mobileMenu').classList.remove('open')">الأنشطة</a>
    <a href="{{ route('news') }}" onclick="document.getElementById('mobileMenu').classList.remove('open')">الأخبار</a>
</div>
...
<div style="position:fixed; top:85px; right:20px; left:20px; z-index:2000; max-width:600px; margin:0 auto;">
    <div style="background:rgba(34,197,94,0.15); border:1px solid rgba(34,197,94,0.4); backdrop-filter:blur(10px); color:#86efac; padding:1rem 1.5rem; border-radius:15px; display:flex; align-items:center; gap:0.8rem; box-shadow:0 10px 30px rgba(0,0,0,0.3); animation: slideIn 0.4s ease-out;">
        ...
        <div style="flex:1; font-size:0.95rem; font-weight:600;">{{ session('success') }}</div>
...
<footer>
    <div class="footer-inner">
        <div class="footer-logo">⚜ {{ $settings->tribe_name ?? 'قبيلة مسونق' }} ⚜</div>
        <p class="footer-desc">{{ $settings->tribe_description ?? 'الموقع الرسمي لقبيلة مسونق' }}</p>
        <div class="footer-links">
            <a href="{{ route('home') }}">الرئيسية</a>
            <a href="{{ route('about') }}">عن القبيلة</a>
            <a href="{{ route('members') }}">الأعضاء</a>
            <a href="{{ route('activities') }}">الأنشطة</a>
        ...
        <div class="footer-copy">
            جميع الحقوق محفوظة &copy; {{ date('Y') }} — {{ $settings->tribe_name ?? 'قبيلة مسونق' }}
        </div>
    </div>
</footer>
