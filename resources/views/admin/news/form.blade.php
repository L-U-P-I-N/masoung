...
@section('title', $item ? 'تعديل خبر' : 'إضافة خبر')
@section('page-title', $item ? 'تعديل الخبر' : 'إضافة خبر جديد')
@section('breadcrumb', 'الرئيسية / الأخبار / ' . ($item ? 'تعديل' : 'إضافة'))

@section('topbar-actions')
    <a href="{{ route('admin.news') }}" class="btn btn-edit">
...
@section('content')
<form method="POST"
    action="{{ $item ? route('admin.news.update', $item->id) : route('admin.news.store') }}"
    ...
    <div class="form-card">
        <div class="form-grid">
            ...
            <div class="form-group full">
                ...
                <input type="text" name="title" value="{{ old('title', $item->title ?? '') }}" placeholder="أدخل عنوان الخبر" required>
            ...
            <div class="form-group full">
                ...
                @if($item && $item->image)
                    @php $existingImages = array_filter(array_map('trim', explode(',', $item->image))); @endphp
            ...
            <div class="form-group full">
                ...
                <textarea name="excerpt" placeholder="ملخص مختصر يظهر في قوائم الأخبار..." required>{{ old('excerpt', $item->excerpt ?? '') }}</textarea>
            ...
            <div class="form-group full">
                ...
                <textarea name="content" style="min-height:250px" placeholder="اكتب تفاصيل الخبر الكاملة هنا..." required>{{ old('content', $item->content ?? '') }}</textarea>
        ...
        <div style="border-top:1px solid var(--border);margin-top:1.5rem;padding-top:1.5rem;display:flex;gap:1rem">
            <button type="submit" class="btn btn-primary" style="padding:0.75rem 2rem;font-size:0.95rem">
                ...
                {{ $item ? 'حفظ التعديلات' : 'نشر الخبر' }}
            </button>
            <a href="{{ route('admin.news') }}" class="btn btn-edit" style="padding:0.75rem 1.5rem">إلغاء</a>
        </div>
    </div>
</form>
@endsection
