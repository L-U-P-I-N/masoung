...
<body>
    <div class="container">
        ...
        <div class="content">
            <p>مرحباً،</p>
            <p>نود إبلاغك بأنه تم تسجيل دخول جديد إلى لوحة إدارة <strong>{{ $tribe_name }}</strong>.</p>
            
            <div class="info-box">
                <p><strong>الاسم:</strong> {{ $admin_name }}</p>
                <p><strong>البريد الإلكتروني:</strong> {{ $admin_email }}</p>
                <p><strong>عنوان IP:</strong> {{ $ip_address }}</p>
                <p><strong>التوقيت:</strong> {{ $timestamp }}</p>
            </div>

            <p>إذا كنت أنت من قام بهذا الإجراء، فلا داعي للقلق. أما إذا لم تقم بذلك، يرجى تغيير كلمة المرور فوراً ومراجعة سجل النشاطات.</p>
            
            <center>
                <a href="{{ route('admin.login') }}" class="btn">الانتقال إلى لوحة التحكم</a>
            </center>
        </div>
        <div class="footer">
